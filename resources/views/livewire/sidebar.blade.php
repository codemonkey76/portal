<div x-data="{admin: @entangle('admin')}" class="flex flex-col min-h-0 bg-gray-800 p-4">
    <div class="flex-1 flex flex-col pt-5 pb-4 overflow-y-auto">
        ...
        <div class="flex space-x-2 mx-auto items-center justify-between">
            ...
            @can('view admin menu')
                <div
                    class="flex items-center px-2 py-2 text-sm leading-5 font-medium focus:outline-none text-gray-300 hover:text-white space-x-2">
                    <button @click="admin = !admin"
                            type="button"
                            class="relative inline-flex flex-shrink-0 h-6 w-11 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
                            :class="{'bg-gray-200': !admin, 'bg-indigo-600': admin}"
                            role="switch"
                            aria-checked="false">
                        <span class="sr-only">Admin menu</span>
                        <span aria-hidden="true"
                              class="pointer-events-none inline-block h-5 w-5 rounded-full bg-white shadow transform ring-0 transition ease-in-out duration-200"
                              :class="{ 'translate-x-5': admin, 'translate-x-0': !admin}"></span>
                    </button>
                    <span>Admin</span>
                </div>
            @endcan
