            ...
            <div class="md:px-6 lg:px-8 py-2">
                <div class="flex justify-between">
                    <div class="flex items-center space-x-2">
                        <h1 class="text-xl font-semibold">Admin Menus</h1>
                        <x-input.search wire:model="search" />
                    </div>
                    <div class="flex space-x-2">
                        <x-input.per-page :options="$perPageOptions" wire:model="perPage" />
                        @can('menus.create')<!-- CreateAdminMenuButton --><x-button.primary wire:click="create">Create</x-button.primary>@endcan
                    </div>
                </div>
            </div>
            <!-- Command Bar -->
                ...
                <x-slot name="body">
                    @forelse ($items as $item)
                        <x-table.row>
                            ...
                            <x-table.cell>
                                @can('menus.update')<!-- EditAdminMenuButton --><x-small-button.warning wire:click="edit({{$item}})">Edit</x-small-button.warning>@endcan
                                @can('menus.destroy')<!-- DeleteAdminMenuButton --><x-small-button.danger wire:click="delete({{$item}})">Delete</x-small-button.danger>@endcan
                            </x-table.cell>
                        </x-table.row>
                    @empty
                        <x-table.row>
